...
void *my_malloc_check(size_t sz);
double
...
double TestGScatAlt( int reps, int len, GOPctx *ctx )
{
   ...
   {
      first_time = 1;
      root_latency = (double*) my_malloc_check(proc_num * sizeof(double));
   }
   else
      first_time = 0;
   oper_lat = (double*) my_malloc_check(proc_num * sizeof(double));
   lower_bound = (double*) my_malloc_check(proc_num * sizeof(double));
