...
#include <stdio.h>
#include <math.h>
/* HUGE_TIME is a maximum time unlikely to be exceeded, in seconds.
   1e8 is more than a year */
#define HUGE_TIME 1.0e8


#include "mpi.h"
#include "mpptest.h"
#include "getopts.h"
/* Keep the rank and size in MPI_COMM_WORLD */
int NUMNODES, MYPROCID;
...
static int Gsizelist[MAX_SIZE_LIST];
static int Gnsizes = 0;
...
TwinResults *InsertElm( TwinResults *, TwinResults *, int );
...
int main( int argc, char *argv[] )
{
    ...
    if (NUMNODES < 2 && !SYArgHasName( &argc, argv, 0, "-memcpy" )) {
...
/* Get the output context */
    ...
    Gnsizes = SYArgGetIntList(&argc, argv, 1, "-sizelist", MAX_SIZE_LIST,
			       Gsizelist);
	...
	while( k <= svals[1] && Gnsizes < MAX_SIZE_LIST ) {
	    Gsizelist[Gnsizes++] = k;
	    k *= 2;
	}
	...
	doDebugAuto = SYArgHasName(&argc, argv, 1, "-autodebug");
    }
...
/* Pick the general test based on the presence of an -gop, -overlap, -bisect
   or no arg */
	...
	TimeScale = 0.5;
	RateScale = (double) NUMNODES; /* * (2 * 0.5) */
    }
		    ...
		    double autorel, void *msgctx)
{
	...
	*/
	...
	int k;
	if (Gnsizes) {
	    twin = AllocResultsArray(Gnsizes);
	    SetResultsForList(Gsizelist, Gnsizes, twin);
	}
	else {
	    Gnsizes = 1 + (last - first)/incr;
	    twin = AllocResultsArray(Gnsizes);
...
/* Add an entry to the list half way (in length) betwen prev and next,
   but with the constraint that the new length must be a multiple of
   baselength.
 */
TwinResults *InsertElm( TwinResults *prev, TwinResults *next, int baselength )
{
    TwinResults *tnew;
    int         deltalen;

    /* First, compute the new length and ensure that it is a multiple
       of baselength.  If this is not possible, return null */
    deltalen = (next->len - prev->len) / 2;
    if (baselength > 1) {
	/* Assumption (and requirement) is that prev->len mod baselength
	   is 0. */
	/* This case should not happen because of how InsertElm is called,
	   but the test is a useful check */
	if (deltalen <= baselength) return NULL;
	if ( (deltalen % baselength) != 0) {
	    deltalen = baselength * (deltalen / baselength);
...
Rerun with a smaller interval or without the -auto option\n" );
    ...
    prev->next  = tnew;
    next->prev  = tnew;
    tnew->len   = prev->len + deltalen;
    tnew->n_avg = next->n_avg;
    tnew->t     = HUGE_TIME;

    if (doDebugAuto) {
	printf( "%d running test with n_avg=%d, len=%d\n", 
		MYPROCID, tnew->n_avg, (int)tnew->len );fflush(stdout); 
    }
		    ...
		    void *msgctx, int min_dx, double autorel )
{
	...
	if (do_refine) {
	    if (doDebugAuto) {
		if (tprev && tnext) {
		    printf("Refining:[%d,%d,%d] with times [%.2e,%.2e,%.2e]\n",
			   tprev->len, twin_p->len, tnext->len,
			   tprev->t, twin_p->t, tnext->t);
		}
		else {
		    printf("Refining at %d with time %.2e (no prev and/or next)",
			   twin_p->len, twin_p->t);
		}
		printf("\tT-est %.2e; offest = %.2e\n", t_center, t_offset);
	    }
	    /* update the list by refining both segments.
	       Note that if the refinement would produce a length that
	       is not a multiple of min_dx, don't make the update; in
	       that case, InsertElm returns NULL.
	       We also do not refine an interval that we just refined.
	       We detect this by checking whether twin_p->prev == tprev.
	       That will be turn unless we have just refined the interval
	       from tprev to twin_p. */
	    if (twin_p->prev&& twin_avail &&
		min_dx < twin_p->len - twin_p->prev->len &&
                tprev == twin_p->prev) {
		if (InsertElm( twin_p->prev, twin_p, min_dx ))
		    n_refined ++; 
	    }
	    if (twin_p->next && twin_avail && 
		min_dx < twin_p->next->len - twin_p->len) {
		if (InsertElm( twin_p, twin_p->next, min_dx ))
		    n_refined ++;
	    }
	}
	tprev  = twin_p;
...
void CheckTimeLimit( void )
{
    ...
    if (MYPROCID == 0 && MPI_Wtime() - start_time > max_run_time) {
