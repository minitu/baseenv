...
int main( int argc, char *argv[] )
{
    ...
    if (MYPROCID == 0) fputs( test_type_msg, stdout );
...
/* Disable resource checking */
	...
	MPI_Allreduce(&bytes_sent, &bytes_so_far, 1, MPI_DOUBLE, MPI_SUM,
		      MPI_COMM_WORLD );
	if (MYPROCID == 0 && nextprint.tv_sec <= currenttime.tv_sec) {
	    char   *str, *p;
	    char   timebuf[100];
	    double rate, delta_t;
...
int AllToAllNB( int pattern, int size )
{
    ...
    rc      = (MPI_Request *)malloc((unsigned)(NUMNODES * sizeof(MPI_Request) ));
    if (!rc) {
	MPI_Abort( MPI_COMM_WORLD, 1 ); 
    }

    buffer  = (unsigned long *)malloc((unsigned)(NUMNODES * size * sizeof(long) ));  if (!buffer)return 0;;
    bufsize = size * sizeof(long);
    bufmsize = bufsize;
    for (sender = 0; sender < NUMNODES; sender++) {
	if (sender != MYPROCID) {
