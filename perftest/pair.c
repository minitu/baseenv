...
void PairChange( int distance, PairData ctx )
{
	...
	proc2 = GetNeighbor( MYPROCID, distance, 0 );
	if (proc2 == 0) {
    ...
    ctx->proc1     = 0;
    ctx->proc2     = proc2;
    ctx->is_master = MYPROCID == ctx->proc1;
    ctx->is_slave  = MYPROCID == proc2;
    if (ctx->is_master) {
	ctx->partner     = proc2;
...
void BisectChange( int distance, PairData ctx )
{
    ...
    ctx->proc1     = NO_NBR;
    ctx->proc2     = NO_NBR;
    ctx->is_master = marks[MYPROCID] == 1;
    ctx->is_slave  = marks[MYPROCID] == 2;
    if (ctx->is_master) {
	ctx->partner = GetNeighbor( MYPROCID, distance, 1 );
	ctx->destination = ctx->partner;
	ctx->source      = ctx->partner;
    }
    else if (ctx->is_slave) {
	ctx->partner = GetNeighbor( MYPROCID, distance, 0 );
	ctx->destination = ctx->partner;
	ctx->source      = ctx->partner;
    }
    else {
...
double round_trip_vector(int reps, int len, PairData ctx)
{
  ...
  comm = MPI_COMM_WORLD;
#if MPI_VERSION >= 3
  MPI_Type_create_resized(MPI_DOUBLE, 0, VectorStride*sizeof(double), &vec);
#else
