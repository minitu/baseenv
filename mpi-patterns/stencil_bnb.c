...
double stencil_bnb(probDesc *pd, procInfo *pi, stencilInfo *si, stencilTime *st)
{
	...
	t = MPI_Wtime();
	MPI_Irecv(rbufnorth, bx, MPI_DOUBLE, pi->north, 9, comm, &reqs[0]);
	MPI_Irecv(rbufsouth, bx, MPI_DOUBLE, pi->south, 9, comm, &reqs[1]);
	MPI_Irecv(rbufeast, by, MPI_DOUBLE, pi->east, 9, comm, &reqs[2]);
	MPI_Irecv(rbufwest, by, MPI_DOUBLE, pi->west, 9, comm, &reqs[3]);

	MPI_Send(sbufnorth, bx, MPI_DOUBLE, pi->north, 9, comm);
	MPI_Send(sbufsouth, bx, MPI_DOUBLE, pi->south, 9, comm);
	MPI_Send(sbufeast, by, MPI_DOUBLE, pi->east, 9, comm);
	MPI_Send(sbufwest, by, MPI_DOUBLE, pi->west, 9, comm);
...
#endif
    MPI_Allreduce(&heat, &finalHeat, 1, MPI_DOUBLE, MPI_SUM, comm);
