...
/* See stencil_mpi_ddt.c in the Advanced MPI tutorial */
double stencil_nb_ddt(probDesc *pd, procInfo *pi, stencilInfo *si, stencilTime *st)
{
	...
	MPI_Request reqs[8];
	t = MPI_Wtime();
	MPI_Isend(&aold[ind(1,1)] /* north */, 1, north_south_type, pi->north,
		  9, comm, &reqs[0]);
	MPI_Isend(&aold[ind(1,by)] /* south */, 1, north_south_type, pi->south,
		  9, comm, &reqs[1]);
	MPI_Isend(&aold[ind(bx,1)] /* east */, 1, east_west_type, pi->east, 9,
		  comm, &reqs[2]);
	MPI_Isend(&aold[ind(1,1)] /* west */, 1, east_west_type, pi->west, 9,
		  comm, &reqs[3]);
	MPI_Irecv(&aold[ind(1,0)] /* north */, 1, north_south_type, pi->north,
		  9, comm, &reqs[4]);
	MPI_Irecv(&aold[ind(1,by+1)] /* south */, 1, north_south_type,
		  pi->south, 9, comm, &reqs[5]);
	MPI_Irecv(&aold[ind(bx+1,1)] /* west */, 1, east_west_type, pi->east,
		  9, comm, &reqs[6]);
	MPI_Irecv(&aold[ind(0,1)] /* east */, 1, east_west_type, pi->west, 9,
		  comm, &reqs[7]);
