             ...
             int *maxcommtypes)
{
    ...
    */
    ...
    pi->pz            = dims[2];     /* 3rd dim processes */
	...
	for (i=1; i<argc; i++) {
		...
		*maxcommtypes = 1;
		...
		fprintf(stderr, "%s --size xxx --energy nn --iters nn --px x --py y -pz z -o filename --norma --normadtype --normacontig --noskip --worldonly\n", argv[0]);
		fflush(stderr);
	...
	if (si->nSizes <= 0) {
	    /* Create a default */
	    getRangeList("16,32,64", &nvals, (int **)&vals);
	    si->nSizes = nvals;
	    si->nSize  = vals;
	}

	if(pi->px * pi->py * pi->pz != size)
	    myAbort(comm, 1, "px*py*pz != size of COMM_WORLD");
	...
	for (i=0; i<nvals; i++) {
	    ...
	    if(vals[i] % pi->pz != 0)
		myAbort(comm, 3, "n mod pz != 0");
	}

	/* distribute arguments */
	args[0] = nvals; args[1] = pd->energy; args[2] = si->niters;
	args[3] = pi->px; args[4] = pi->py; args[5] = pi->pz;
	args[6] = pi->useRMA;
	args[7] = pi->useRMAdtype; args[8] = pi->skipCongruent;
	args[9] = pi->withCart; args[10] = *maxcommtypes;
	...
	pi->px=args[3]; pi->py=args[4]; pi->pz=args[5]; pi->useRMA=args[6];
	pi->useRMAdtype=args[7];
	pi->skipCongruent = args[8];
	pi->withCart      = args[9];
	*maxcommtypes = args[10];
...
void getDecomp(MPI_Comm comm, procInfo *pi, stencilInfo *si)
{
    ...
    /* determine my coordinates (x,y,z) -- r=x*b*c+y*c+z in the
       3d processor array, with dims (a,b,c) */
    r    = rank;
    s    = pi->px * pi->py;
    rz   = si->rz = r / s;
    r    = r - rz * s;
    ry   = si->ry = r / pi->px;
    rx   = si->rx = r - ry * pi->px;
    /* determine my six neighbors */
    pi->north  = rank-pi->px; if(ry-1 < 0)   pi->north = MPI_PROC_NULL;
    pi->south  = rank+pi->px; if(ry+1 >= pi->py) pi->south = MPI_PROC_NULL;
    pi->west   = rank-1;      if(rx-1 < 0)   pi->west = MPI_PROC_NULL;
    pi->east   = rank+1;      if(rx+1 >= pi->px) pi->east = MPI_PROC_NULL;
    pi->bottom    = rank+pi->px*pi->py;  if (rz+1 >= pi->pz) pi->bottom = MPI_PROC_NULL;
    pi->top = rank-pi->px*pi->py; if (rz-1 < 0) pi->top = MPI_PROC_NULL;
    /* decompose the domain */
    si->bx   = si->n/pi->px; /* block size in x */
    si->by   = si->n/pi->py; /* block size in y */
    si->bz   = si->n/pi->pz; /* block size in z */
    si->offx = rx*si->bx;  /* offset in x */
    si->offy = ry*si->by;  /* offset in y */
    si->offz = rz*si->bz;  /* offset in z */

    /* Reality check */
    {int err = 0;
    if (pi->north != MPI_PROC_NULL && (pi->north < 0 || pi->north >= size)) {
	err++;
	fprintf(stderr, "north = %d\n", pi->north);
    }
    if (pi->south != MPI_PROC_NULL && (pi->south < 0 || pi->south >= size)) {
	err++;
    ...
    if (pi->top != MPI_PROC_NULL && (pi->top < 0 || pi->top >= size)) {
	err++;
	fprintf(stderr, "top = %d\n", pi->top);
    }
    if (pi->bottom != MPI_PROC_NULL && (pi->bottom < 0 || pi->bottom >= size)) {
	err++;
...
void getDecompCart(MPI_Comm comm, procInfo *pi, stencilInfo *si)
{
    ...
    MPI_Dims_create(size, 3, pdims);
    pi->px = pdims[0];
    pi->py = pdims[1];
    pi->pz = pdims[2];

    /* Select reordering to let system order ranks */
    MPI_Cart_create(comm, 3, pdims, periods, 1, &pi->comm);
    ...
    MPI_Cart_coords(pi->comm, crank, 3, coords);
    si->rx = coords[0];
    si->ry = coords[1];
    si->rz = coords[2];
    ...
    MPI_Cart_shift(pi->comm, 2, 1, &pi->top,   &pi->bottom);
    ...
    si->offz = si->rz*si->bz;  /* offset in z */
}

void getDecompNodecart(MPI_Comm comm, procInfo *pi, stencilInfo *si)
{
    int    rank, size, nrank;
    int    pdims[3] = {0,0,0};
    int    periods[3] = {0,0,0};
    ...
    /* Select reordering to let system order ranks */
    MPIX_Nodecart_create(comm, 3, pdims, periods, 1, &pi->comm);
    MPI_Comm_set_name(pi->comm, "Nodecart");

    /* determine my coordinates (x,y) -- r=x*a+y in the 2d processor array */
    MPI_Comm_rank(pi->comm, &nrank);
    MPIX_Nodecart_coords(pi->comm, nrank, 3, coords);
    si->rx = coords[0];
    si->ry = coords[1];
    si->rz = coords[2];
    ...
    MPIX_Nodecart_shift(pi->comm, 2, 1, &pi->top,   &pi->bottom);
...
void initProbDesc(probDesc *pd, stencilInfo *si)
{
    ...
    int sources[NSOURCES][3]; /* = {{n/2,n/2,n/2}, {n/3,n/3,n/3},
				 {n*4/5,n*8/9,n*4/5}}; */
	...
	if(locx >= 0 && locx < si->bx && locy >= 0 && locy < si->by &&
	   locz >= 0 && locz < si->bz) {
	    ...
	    pd->locsources[pd->locnsources][2] = locz+1; /* offset by halo zone */ 	    pd->locnsources++;
	}
    }
}
...
void allocMesh(stencilInfo *si)
{
    /* Allocate memory in a single block; this simplifies some of the
       versions (esp. RMA). */
    size_t asize = (si->bx+2)*(si->by+2)*(si->bz+2);
...
{
    initMeshBase(pd, si->bx, si->by, si->bz, si->aold, si->anew);
}

void initMeshBase(probDesc *pd, int bx, int by, int bz, double *restrict aold,
		   double *restrict anew)
{
	...
	aold[ind(pd->locsources[i][0],pd->locsources[i][1],pd->locsources[i][2])] +=
...
void printSoln(stencilInfo *si, double *v, const char *name)
{
    char fname[256];
    FILE *fp;
    int  bx = si->bx, by = si->by, bz = si->bz, n = si->n;
...
void getNodeCommunicationStats(procInfo *pi, FILE *fp)
{
    ...
    if (pi->top    != MPI_PROC_NULL) sends[ns++] = pi->top;
    if (pi->bottom != MPI_PROC_NULL) sends[ns++] = pi->bottom;
...
#if 0
void printDecomp(FILE *fp, MPI_Comm comm, procInfo *pi, stencilInfo *si)
{
    int crank, wrank;
    ...
    MPI_Comm_rank(MPI_COMM_WORLD, &wrank);
    seqBegin(pi->comm);
    fprintf(fp, "%d(w%d): procInfo [%d,%d,%d] %d %d %d %d %d %d\n",
	    crank, wrank, pi->px, pi->py, pi->pz,
	    pi->north, pi->south, pi->east, pi->west, pi->top, pi->bottom);
    fprintf(fp, "%d(w%d): stencilInfo [%d,%d,%d] at (%d,%d,%d) pcoords (%d,%d,%d)\n",
	    crank, wrank, si->bx, si->by, si->bz, si->offx, si->offy, si->offz,
	    si->rx, si->ry, si->rz);
    fflush(fp);
    seqEnd(pi->comm);
}
