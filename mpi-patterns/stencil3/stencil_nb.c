/*
 * Simple nonblocking version with no overlap
 */
#include "mpi.h"
#include <stdio.h>
#include <stdlib.h>
...
double stencil_nb(probDesc *pd, procInfo *pi, stencilInfo *si, stencilTime *st)
{
	...
	t = MPI_Wtime();
	MPI_Isend(sbufnorth, bx*bz, MPI_DOUBLE, pi->north, 9, comm, &reqs[0]);
	MPI_Isend(sbufsouth, bx*bz, MPI_DOUBLE, pi->south, 9, comm, &reqs[1]);
	MPI_Isend(sbufeast, by*bz, MPI_DOUBLE, pi->east, 9, comm, &reqs[2]);
	MPI_Isend(sbufwest, by*bz, MPI_DOUBLE, pi->west, 9, comm, &reqs[3]);
	MPI_Isend(sbuftop, bx*by, MPI_DOUBLE, pi->top, 9, comm, &reqs[4]);
	MPI_Isend(sbufbottom, bx*by, MPI_DOUBLE, pi->bottom, 9, comm, &reqs[5]);
	MPI_Irecv(rbufnorth, bx*bz, MPI_DOUBLE, pi->north, 9, comm, &reqs[6]);
	MPI_Irecv(rbufsouth, bx*bz, MPI_DOUBLE, pi->south, 9, comm, &reqs[7]);
	MPI_Irecv(rbufeast, by*bz, MPI_DOUBLE, pi->east, 9, comm, &reqs[8]);
	MPI_Irecv(rbufwest, by*bz, MPI_DOUBLE, pi->west, 9, comm, &reqs[9]);
	MPI_Irecv(rbuftop, bx*by, MPI_DOUBLE, pi->top, 9, comm, &reqs[10]);
	MPI_Irecv(rbufbottom, bx*by, MPI_DOUBLE, pi->bottom, 9, comm, &reqs[11]);
	...
	t = MPI_Wtime();
	/* Unpack loops into ghost cells for each face */
        if (pi->north != MPI_PROC_NULL) {
	    p=0;
	    for (k=0; k<bz; ++k)
