...
/* These are grouped to encourage the compiler to remove common
   subexpressions in loops */
#ifdef USE_ROW_MAJOR
#define ind(i,j,k) (k)*((by+2)*(bx+2))+(j)*(bx+2)+(i)
#define NS_CONTIG 1
#elif defined(USE_COLUMN_MAJOR)
#define ind(i,j,k) (i)*((by+2)*(bz+2))+(j)*(bz+2)+(k)
#define EW_CONTIG 1
...
typedef struct {
    int      px, py, pz;        /* Processor in x, y, z dimensions */
    int      north, south, east, west,
	top, bottom; /* rank of Neighbors in comm */
...
typedef struct {
    ...
    int locsources[NSOURCES][3]; /* sources local to my rank */
...
typedef struct {
	...
	bx, by, bz,        /* Block sizes (local mesh size) */
	offx, offy, offz,  /* (x,y,z) index for upper left corner of mesh */
	rx, ry, rz;        /* (x,y,z) index for process in process mesh */
