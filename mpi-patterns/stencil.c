...
int main(int argc, char *argv[])
{
	...
	fprintf(fp, "stencil for baseenv version %s (%s)\n",
		PACKAGE_VERSION, BASEENV_GITHASH);
	fprintf(fp, "Processor name %s\n", name);
	fprintf(fp, "WORLD size %d; processor mesh is %d x %d\n",
		wsize, pi.px, pi.py);
	fflush(fp);
    }
	...
	for (sizeidx=0; sizeidx < si.nSizes; sizeidx++) {
	    ...
	    /* 4 adds, 2 multiplies, 1 add for total heat */
	    nwork = (long)si.n * (long)si.n * 6;
#if COMPUTE_HEAT_EACH_ITERATION
	    /* Add the cost of summing the heat within the stencil loop */
	    nwork += (long)si.n * (long)si.n;
#endif
	    nwork *= si.niters;
		...
		break;
	    case 2:
		getDecompNodecart(MPI_COMM_WORLD, &pi, &si, 0);
		...
		break;
	    case 3:
                /* Include socket information when forming cart */
		getDecompNodecart(MPI_COMM_WORLD, &pi, &si, 1);
		if (wrank == 0 && sizeidx == 0) {
